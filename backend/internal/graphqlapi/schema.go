...
func New(d Deps) (graphql.Schema, error) {
	...
	modelType := graphql.NewObject(graphql.ObjectConfig{
		...
		Fields: graphql.Fields{
			...
			"softCeilingStart": &graphql.Field{Type: graphql.Float, Resolve: field("SoftCeilingStart"),
				Description: "Above this the scale saturates, so the maximum is approached but never reached and top guests stay rankable."},
			"softFloorStart": &graphql.Field{Type: graphql.Float, Resolve: field("SoftFloorStart")},
